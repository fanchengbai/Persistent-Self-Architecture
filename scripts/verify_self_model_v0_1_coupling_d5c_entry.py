...
from psa.self_model.d5c_real_mechanism import build_d5c_entry_static_report


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Coupling-D5C no-model real-mechanism entry"
    )
    parser.add_argument("--config", required=True)
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    report = build_d5c_entry_static_report(
