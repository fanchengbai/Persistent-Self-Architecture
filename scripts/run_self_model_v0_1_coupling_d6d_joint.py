...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the one future-authorized D6D joint projection training and noncore pilot"
    )
    ...
    if config_path != (root / CONFIG_RELATIVE_PATH).resolve():
        raise PermissionError("D6D config path is not frozen")
    if authorization_path != (root / AUTHORIZATION_RELATIVE_PATH).resolve():
        raise PermissionError("D6D authorization path is not frozen")
    if output_dir != (root / OUTPUT_RELATIVE_DIR).resolve():
        raise PermissionError("D6D output path is not frozen")
    if authorization_path.exists():
        raise FileExistsError("D6D machine authorization already exists")
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError("D6D output directory is not empty")
    authorization = build_d6d_authorization(
        config_path=config_path,
        project_root=root,
        authorization_text=FUTURE_EXECUTION_AUTHORIZATION_TEXT,
    )
    write_json_exclusive(authorization_path, authorization)
    report = run_d6d_real_joint(
        config_path=config_path,
    ...
    print(json.dumps({
        ...
        "projection_artifact_sha256": report["projection_artifact_sha256"],
