...
GATE_CONFIG="${PROJECT_ROOT}/configs/gates/impl3m_g1h_2.9b_checkpoint_roundtrip.dev.json"
...
OUTPUT_DIR="${PSA_IMPL3M_OUTPUT:-${PROJECT_ROOT}/results/development/impl3m_g1h_2.9b_checkpoint_roundtrip}"
...
"${PYTHON_BIN}" -m psa checkpoint-roundtrip-gate \
  --config "${MODEL_CONFIG}" \
  --gate-config "${GATE_CONFIG}" \
  --output-dir "${OUTPUT_DIR}" \
  --project-root "${PROJECT_ROOT}"

echo "Impl-3m G1h 2.9B checkpoint roundtrip gate finished."
