...
GATE_CONFIG="${PROJECT_ROOT}/configs/gates/impl3na_g1h_2.9b_reset_stability.dev.json"
...
OUTPUT_DIR="${PSA_IMPL3NA_OUTPUT:-${PROJECT_ROOT}/results/development/impl3na_g1h_2.9b_reset_stability}"
...
"${PYTHON_BIN}" -m psa reset-stability-diagnostic \
  --config "${MODEL_CONFIG}" \
  --gate-config "${GATE_CONFIG}" \
  --output-dir "${OUTPUT_DIR}" \
  --project-root "${PROJECT_ROOT}"

echo "Impl-3n-a G1h 2.9B reset stability diagnostic finished."
