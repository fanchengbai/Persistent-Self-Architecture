...
from psa.self_model.d6a_persistent_dispatcher import build_d6a_report


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the pure-Python Coupling-D6A persistent dispatcher"
    )
    parser.add_argument("--config", required=True)
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    report = build_d6a_report(
