...
GATE_CONFIG="${PROJECT_ROOT}/configs/gates/impl3nb_g1h_2.9b_state_operations_warmed.dev.json"
...
OUTPUT_DIR="${PSA_IMPL3NB_OUTPUT:-${PROJECT_ROOT}/results/development/impl3nb_g1h_2.9b_state_operations_warmed}"
...
"${PYTHON_BIN}" -m psa state-operations-gate \
  --config "${MODEL_CONFIG}" \
  --gate-config "${GATE_CONFIG}" \
  --output-dir "${OUTPUT_DIR}" \
  --project-root "${PROJECT_ROOT}"

echo "Impl-3n-b warmed G1h 2.9B state operations gate finished."
