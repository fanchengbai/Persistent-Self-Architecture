...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the no-model D8-C real numerical-identifiability safe entry"
    )
    parser.add_argument("--config", default=CONFIG_RELATIVE_PATH)
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    report = build_static_report(config_path=args.config, project_root=args.project_root)
        ...
        json.dumps(
            {
                ...
                "fake_acceptance_checks": len(report["fake_acceptance"]["checks"]),
                "future_forward_calls": report["fake_acceptance"]["counts"][
                    "total_future_forward_calls"
                ],
