...
15. Stage B只读live preflight与机器授权锁已经本地完成：预检绑定干净main提交、设计与
    protocol digest、Stage A原始证据、模型配置与资产哈希、主机环境、224条固定计划及空
    输出目录；只校验模型文件，不加载模型。授权builder只接受固定逐字文本，并绑定design、
    preflight和Stage A digest。Stage B专项29项、全项目258项通过。下一步只在云端生成一次
    不覆盖的只读preflight证据；通过后必须停下来等待项目负责人新的逐字授权，不能从“继续”
    推断模型执行权限。
