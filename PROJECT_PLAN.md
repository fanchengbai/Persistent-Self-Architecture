...
4. B-Dev1与B-Dev2 v0.2云端均已通过；v0.1失败保留。未确认候选已在云端生成，
   锁定12个源码、22个证据文件，checksum=`c7a69971…a3eb`；
5. self digest、payload root、安全边界及34个文件检查已全部通过；项目负责人已逐字
   确认完整checksum，授权仅限升级最终预注册包；
6. 最终预注册包和11,008条补充集已分别持久化并完成跨主机独立复核；正式运行
   基础设施已经实现。三个GPU入口的自包含RWKV运行标志已修复，修复后的云端
   非Core真实模型开发门和只读正式预检均通过；
7. 最终preflight digest=`bc91b2b3cd7557cc2000d8990fbc638de38a6d9895891e8b5b944e9572ab13a6`，
   已绑定模型、最终预注册包、父Core Set、补充集、运行源码、开发门证据、主机环境和
   固定运行标志。项目负责人已逐字确认该checksum并授权固定2.9B模型、320组和
   11,008条记录的完整运行；只允许全量结束并通过原始包验证后观察结果，禁止修改
   冻结设计、自动重跑和重跑EXP-001。下一步保持云端提交不变，建立机器授权记录后
   启动正式runner；运行期间只监控完成组数，不读取派生结果；
...
14. 真实执行runner安全外壳已经离线完成：授权验证默认关闭，独占single-use claim在
    backend前消费，结果和验证报告原子写入；无效结果保留claim并禁止自动重入。独立验证
    逐条核对224条路由但不计算准确率或研究决定。Stage B专项23项、全项目252项通过。
    下一步等待确认实现服务器只读live preflight与机器授权验证，仍不运行模型。
15. Stage B只读live preflight与机器授权锁已经完成并在云端通过：预检绑定干净main提交、设计与
    protocol digest、Stage A原始证据、模型配置与资产哈希、主机环境、224条固定计划及空
    输出目录；只校验模型文件，不加载模型。授权builder只接受固定逐字文本，并绑定design、
    preflight和Stage A digest。Stage B专项29项远程测试、全项目258项本地测试通过；云端
    预检失败检查为空，模型未加载或执行。最终文档提交后须生成新的v02证据，以该digest
    进入项目负责人逐字授权门；不能从“继续”推断模型执行权限。
16. 项目负责人已使用冻结原文逐字授权Stage B非Core 224条pilot及本轮结果观察，明确禁止
    Stage A重跑、正式测试集、正式运行、确认性决定和自动重跑。新增单次启动入口，以独占
    方式创建不进Git的机器授权文件并在backend前消费claim。下一步在最终提交上生成
    preflight_v03后启动一次；无论失败或完成均不自动重跑。
17. Stage B单次224条运行已经结束并通过原始包验证，结果digest=`e0b871e7…8b655`，
