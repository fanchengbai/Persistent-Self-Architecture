...
- 项目阶段：Phase 2 — EXP-001B正式运行基础设施云端资格验证
...
6. 最终预注册包和11,008条补充集已分别持久化并完成跨主机独立复核；正式运行
   基础设施已经实现。云端非Core runner开发门曾通过，但首次预检因入口脚本未自行
   固定RWKV运行标志而安全失败；三个GPU入口现已修复，须重跑非Core门和只读预检；
7. 云端预检必须同时绑定模型、最终预注册包、父Core Set、补充集、运行源码、
   开发门证据和主机环境。取得新checksum后必须停在项目负责人独立授权门；
