...
- 项目阶段：Phase 2 — EXP-001B B-Dev2 v0.2正式形状非Core复验
...
4. B-Dev1云端已通过；B-Dev2 v0.1失败审计确认通用runner历史与正式形状norm
   阈值分布错配。v0.2改用64条正式形状非Core题，保持原阈值并写入新目录；
5. 只有B-Dev2 v0.2通过后才生成独立预注册候选checksum；不得复用EXP-001的运行授权，
