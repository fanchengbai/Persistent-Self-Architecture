...
- 项目阶段：Phase 2 — EXP-001B补充集生成前工具与授权门准备
...
6. 最终预注册包已由提交`e9c3528`持久化并完成跨主机独立复核；下一步只实现
   补充集生成前预检、确定性生成器、完整性验证与独立授权锁。不得复用EXP-001的运行授权，
   不重跑EXP-001主要条件，也不重估E1–E3；
7. Phase 3可以继续框架和接口设计，但在Gate 4闭合前不启动显式Self Model的
   正式确认性实验；
8. EXP-001B完整运行、验证和冻结分析后，再作一次明确的Phase 2 Go / Revise / Stop决策。
