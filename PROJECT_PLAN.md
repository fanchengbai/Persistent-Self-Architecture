...
- 项目阶段：Phase 2 — EXP-001B预注册候选云端生成与checksum审阅
...
4. B-Dev1与B-Dev2 v0.2云端均已通过；v0.1失败保留。未确认候选构建与验证代码
   已完成154项本地测试，下一步只在云端生成候选checksum，不生成补充集；
5. 候选会同时锁定设计、v0.1失败、v0.2成功和安全边界；经技术验证及项目负责人
   逐字确认后才可升级。不得复用EXP-001的运行授权，
