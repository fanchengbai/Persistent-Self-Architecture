...
> 最后更新：2026-08-04
> 当前节点：EXP-001B补充集已Git持久化并跨主机复核通过；进入正式运行基础设施准备
...
| 38h. EXP-001B正式运行准备 | 🟡 未授权，仅可开发基础设施 | 实现只读正式预检、完整runner、原始包验证与独立运行授权锁，并先用非补充集夹具回归 | 已有“试卷”不等于允许“考试”；运行源码、模型、冻结包和资源状态必须形成新的预检checksum | 尚未实现EXP-001B正式runner入口，未创建运行授权、未加载模型、未运行或观察结果 | Codex下一步实现；项目负责人后续授权 |
...
14. 下一步只实现正式运行基础设施；正式运行仍需未来另一份独立授权。
