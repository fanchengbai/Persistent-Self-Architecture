...
> 最后更新：2026-08-04
> 当前节点：EXP-001B首次正式预检在运行标志检查处安全失败；入口脚本已修复，等待重跑非Core门和预检
...
| 38h. EXP-001B正式运行准备 | 🟡 云端开发门通过，等待只读预检 | 已实现只读正式预检、三类记录完整runner、320组原子保存/显式续跑、原始包验证与独立运行授权锁；云端真实2.9B非Core门已覆盖全部路由 | 已有“试卷”不等于允许“考试”；运行源码、模型、冻结包和资源状态必须形成新的预检checksum | 开发门`valid=true`：16条matched、16条生成和8条控制共40条，三类路由与8条件全部覆盖；加载7.64秒、运行18.66秒、峰值显存6,311,951,360字节。Core/补充集均未访问，无正式授权、准确率、中间决策、运行或结果观察。下一步只运行非推理预检 | Codex实现；项目负责人云端运行 |
| 38h-a. 首次正式预检运行标志诊断 | ⚠️ 安全失败，修复待复验 | 预检唯一失败项为`runtime_flags_frozen`；审计三个新GPU入口是否自包含固定RWKV运行模式 | 新终端不应依赖旧shell遗留变量；否则同一脚本在不同会话会得到不同preflight | 首次digest=`3d48b04b…0365`作废且不得授权。失败发生在模型加载和trial评分前，runner证据仍有效；正式授权/运行/结果观察全为false。已给开发门、预检和正式运行脚本补齐`RWKV_V7_ON=1`、`RWKV_JIT_ON=0`、`RWKV_CUDA_ON=0`及项目`PYTHONPATH`；新增入口自包含测试后全项目176项通过。脚本哈希变化后须重跑非Core门，再重跑预检 | 项目负责人回传；Codex诊断修复 |
...
EXP-001B补充控制
    ...
    🟡 下一步只实现正式预检/runner/原始包验证/独立运行锁，不运行实验
    ⏸️ 补充测试集生成和正式运行仍分别未授权
正式 state 因果实验
...
15. 非Core正式runner开发门曾在云端通过，但首次正式预检发现入口脚本未自行固定三个RWKV运行标志，唯一失败项为`runtime_flags_frozen`；
16. 三个GPU入口已补齐自包含运行标志。由于脚本属于哈希清单，必须先重跑一次非Core门，再重跑不加载模型的只读正式预检；失败digest `3d48b04b…0365`永久作废；
17. 在项目负责人逐字确认该新checksum和完整运行范围前，不创建运行授权，不执行正式EXP-001B，也不观察任何结果。
