...
> 最后更新：2026-08-04
> 当前节点：EXP-001B补充集生成预检已通过；等待项目负责人确认预检checksum并单独授权生成
...
| 38g. EXP-001B补充集生成门 | 🟡 预检通过，等待授权 | 已实现并在云端运行只读包预检；生成器、包级完整性验证和“绑定实时预检digest的负责人授权文件+执行环境锁”均已就绪 | 最终预注册包只是规则冻结，不等于可以生成正式补充记录；主机或源码状态变化后旧授权也不得复用；生成和运行必须继续分权 | 云端6项检查全部通过，`valid=true`，预检digest=`1f117d5c…fa65`；预算固定320组、5,120+5,120+768=11,008条。模型未加载、试题未评分、补充集未生成，实验未授权/未运行/未观察 | 项目负责人云端预检；等待负责人授权 |
...
EXP-001B补充控制
    ...
    ✅ 云端生成前预检通过，checksum=`1f117d5c…fa65`
    🟡 等待负责人确认该checksum并单独授权生成11,008条记录
...
9. 补充集生成前预检已在云端通过，digest=`1f117d5cd0e9d37706c50bc10db37bf826eaa7166a366589e8ab4121499cfa65`；
10. 生成器、完整性验证和独立授权锁已经就绪，但补充集尚未生成；
11. EXP-001B补充集生成和正式运行仍分别需要后续独立授权，当前均不得执行。
...
| 2026-08-04 | EXP-001B补充集生成前云端预检通过：最终预注册包与digest、父Core Set包与digest、父包未运行边界及11,008条记录预算共6项检查全部为true；`valid=true`，preflight digest=`1f117d5cd0e9d37706c50bc10db37bf826eaa7166a366589e8ab4121499cfa65`。预检明确记录模型未加载、补充trial未评分、补充集未生成，生成授权、实验授权、运行和结果观察仍全部为false。流程只推进到负责人checksum授权门，不创建授权文件、不启动生成器 | 云端`results/development/exp001b_set_preflight/preflight.json` |
