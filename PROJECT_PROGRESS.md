...
> 最后更新：2026-08-04
> 当前节点：EXP-001B加固版生成预检已通过；等待项目负责人确认checksum并单独授权生成
...
| 38g. EXP-001B补充集生成门 | 🟡 加固版预检通过，等待授权 | 加固版云端预检同时验证冻结包、11,008条预算和12个生成相关源码/配置/Schema/脚本SHA-256，并将它们统一绑定进checksum | 防止生成逻辑变化后复用旧授权；生成和正式实验继续保持两次独立授权 | 7项检查全部通过，`valid=true`，有效预检digest=`dbb17d97…2ae6`；旧`1f117d5c…fa65`明确废止。模型未加载、试题未评分、补充集未生成，实验未授权/未运行/未观察 | 项目负责人云端复验；等待负责人授权 |
...
EXP-001B补充控制
    ...
    🟡 等待负责人确认新checksum=`dbb17d97…2ae6`并单独授权生成
...
12. EXP-001B补充集生成和正式运行仍分别需要后续独立授权；当前只可请求生成授权，不得启动正式实验。
