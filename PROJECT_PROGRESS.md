...
> 最后更新：2026-08-04
> 当前节点：EXP-001B补充集生成基础设施已完成本地验证；等待云端预检及负责人确认其checksum
...
| 38g. EXP-001B补充集生成门 | 🟡 工具完成，等待预检与授权 | 已实现只读包预检、11,008条确定性记录生成器、包级完整性验证，以及“绑定实时预检digest的负责人授权文件+执行环境锁”双重开关 | 最终预注册包只是规则冻结，不等于可以读取EXP-001 Core参照并生成正式补充记录；主机或源码状态变化后旧授权也不得复用；生成和运行必须继续分权 | 新增3个命令入口与3个云端脚本；D5控制manifest精确复用，96个控制源组无放回且四状态各24个，8条件路由显式固定；10项新测试及全项目169项回归通过。正式补充集仍为未生成，实验未授权/未运行/未观察 | Codex实现；项目负责人后续授权 |
...
EXP-001B补充控制
    ...
    🟡 等待云端预检，再由负责人确认预检checksum并单独授权生成11,008条记录
...
9. 补充集生成前预检、确定性生成器、完整性验证和独立授权锁已经完成本地测试，尚未执行正式生成；
10. EXP-001B补充集生成和正式运行仍分别需要后续独立授权，当前均不得执行。
