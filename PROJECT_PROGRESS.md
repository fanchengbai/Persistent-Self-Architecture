...
截至2026-08-03，项目负责人已经确认EXP-001B设计草案中的B1–B7。
新增范围仍锁在11,008条控制记录，并明确不重跑EXP-001、不重估E1–E3、
不复用旧授权。两个非Core开发门已经本地实现，当前依次执行：

1. 云端运行B-Dev1：用64个非Core案例验证matched-context精确token配对，并冻结逐组件state norm阈值；
2. 仅当B-Dev1通过，云端运行B-Dev2：验证8条件控制路由、greedy生成格式探针、原子写入和断点恢复；
