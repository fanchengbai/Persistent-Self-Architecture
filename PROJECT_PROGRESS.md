...
> 最后更新：2026-08-04
> 当前节点：EXP-001B正式运行基础设施已实现并完成本地回归；等待云端非Core运行器门与只读预检
...
| 38h. EXP-001B正式运行准备 | 🟡 本地实现完成，等待云端开发门 | 已实现只读正式预检、三类记录完整runner、320组原子保存/显式续跑、原始包验证与独立运行授权锁，并提供非Core真实模型开发门 | 已有“试卷”不等于允许“考试”；运行源码、模型、冻结包和资源状态必须形成新的预检checksum | 320组被固定映射为224组×32条和96组×40条，共11,008条；正式runner不计算准确率或中间决策，完成后禁止自动重跑。新增5项专项测试，全项目175项通过。尚未创建运行授权、未加载正式数据评分、未运行或观察结果 | Codex实现；项目负责人后续云端开发门与授权 |
...
15. 新增非Core正式runner开发门，必须先在云端覆盖matched-context、生成读出和8条件控制三类路由；
16. 开发门通过后运行不加载模型的只读正式预检，得到绑定当前代码、模型、冻结包与主机环境的新checksum；
