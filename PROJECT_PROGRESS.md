...
> 最后更新：2026-08-04
> 当前节点：EXP-001B首次生成预检通过后完成源码绑定加固；等待云端更新并重新预检
...
| 38g. EXP-001B补充集生成门 | 🟡 首次预检通过，源码绑定加固后待复验 | 首次云端预检已验证冻结包与预算；授权前审计又将12个生成相关源码、配置、Schema和脚本的SHA-256清单纳入预检digest | 只绑定数据包而不绑定生成器，修改代码后仍可能复用旧授权；必须让任何生成逻辑变化都会产生新checksum | 首次digest=`1f117d5c…fa65`只证明包与预算有效，现保留为被安全审计取代的记录，不用于授权。加固版新增源码清单完整性检查，11项专项及全项目170项测试通过；模型仍未加载、试题未评分、补充集未生成 | Codex加固；项目负责人云端更新后复验 |
...
EXP-001B补充控制
    ...
    ⚠️ 首次云端预检通过但未覆盖生成器源码；checksum=`1f117d5c…fa65`不用于授权
    ✅ 预检已加固为绑定12个生成源码/配置/脚本SHA-256
    🟡 等待云端更新后重新预检，再由负责人确认新checksum
...
9. 首次云端预检通过，digest=`1f117d5cd0e9d37706c50bc10db37bf826eaa7166a366589e8ab4121499cfa65`，但授权前审计发现它未绑定生成器源码，因此不得用于授权；
10. 加固版预检已纳入12个生成相关源码、配置、Schema和脚本digest，等待云端更新后重新运行；
11. 生成器、完整性验证和独立授权锁已经就绪，但补充集尚未生成；
12. EXP-001B补充集生成和正式运行仍分别需要后续独立授权，当前均不得执行。
