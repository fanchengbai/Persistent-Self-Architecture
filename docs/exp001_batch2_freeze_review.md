...
> 版本：v0.2
> 状态：Review Candidate；D1–D3 与 B1 已解决；不是预注册文件
> 日期：2026-07-31
...
- **Go：冻结正式模板、控制任务、seeds 与统计协议候选。**
- **Hold：不得进入确认性 Batch 4。**
- **原因：状态工程门和历史写入协议已经闭合，但正式模板、控制任务、统计模拟和不可变预注册包仍未冻结。**

这一区分保证我们不会把“工程工具可用”误写成“研究假设已经成立”。

## 8. 下一轮共同确认的推荐方案

下面是 B2–B5 的具体默认方案。它们尚未视为最终冻结；项目负责人确认后，
Codex 才会实现正式生成器、资格门、统计模拟和不可变预注册包。
...
建议原样接受评价协议 v0.1 中在正式结果之前写下的默认值：
...
建议保留 `N=320` 个独立 factorial groups，不下调。实现完整生成器后：

1. 使用开发阶段 nuisance variance 做不少于 10,000 次模拟；
...
这仍然不是正式实验。只有不可变预注册包再次得到人工确认，Core Set 才能
生成或解封。
