...
本轮没有建立 runner、backend 或模型执行入口。进入任何 Stage B 模型执行前，必须另行
完成并审查：

1. Stage B 纯离线 runner/backend 集成与 fake-adapter 测试；
2. 只读服务器 preflight，核验 Stage A 原始结果、当前 Git commit、模型资产和主机环境；
3. 结果 schema 与原子输出/单次运行锁；
...
本文件、draft config、schema、manifest builder 和测试只属于离线设计与风险审查。
它们不是 Stage B 执行授权，不创建测试集，不加载模型，不观察新的模型结果，也不改变
EXP-001B 或任何确认性决定。
